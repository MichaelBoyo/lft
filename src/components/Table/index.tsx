...
const Table: FC<ITable> = () => {
  ...
  const orgNames: string[] = rows
    ? rows.map((data: any) => String(data.orgName))
    : [""];
        ...
        rows?.filter(
          (row: any) =>
            ...
            (filterQuery.userName === ""
              ...
              : row.userName
                  .toLowerCase()
                  ?.includes(filterQuery.userName?.toLowerCase())) ||
            (filterQuery.email === ""
              ? false
              : row.email
                  .toLowerCase()
                  ?.includes(filterQuery.email?.toLowerCase())) ||
            filterQuery.phoneNumber === row.phoneNumber ||
            filterQuery.createdAt === row.createdAt ||
            filterQuery.status
              ?.map((val) => val.toLowerCase())
              .includes(row.status) ||
            (filterQuery.status?.includes("Blacklisted") &&
              blackListed?.includes(row.userName))
        )
      );
    } else {
  ...
  if (isError) {
    return (
      <div className={style.Table__servererror}>
        <h1 className={style.Table__servererror__heading}>
          Unable To reach servers :(
        </h1>
        <p className={style.Table__servererror__text}>
    ...
    <>
      ...
      <div className={style.Table__footer}>
        <div className={style.Table__footer__select}>
          ...
          <span>out of {filteredData ? filteredData.length : 100}</span>
        </div>
